...
void submenu_inventory_edit(void) {
    ...
    utils_trim(qty_buffer);
    new_p.quantity = strlen(qty_buffer) == 0 ? -1 : atoi(qty_buffer);
...
void submenu_sales_create(void) {
    ...
    utils_clear_screen();
    utils_print_header("DATOS DEL CLIENTE");
    utils_get_string_input("Nombre del cliente: ", t.customer_name, MAX_NAME);
    if (strlen(t.customer_name) == 0) {
